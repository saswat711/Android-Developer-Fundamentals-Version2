...
import com.example.sharedprefs.databinding.ActivitySettingsBinding;
...
public class SettingsActivity extends Activity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.backgroundSpinner.setOnItemSelectedListener(this);
        binding.backgroundSpinner.setAdapter(adapter);
        binding.saveCountSwitch.setChecked(mPreferences.getBoolean(COUNT_SAVE_KEY, true));
        String savedSpinnerSetting = mPreferences.getString(SELECTED_SPINNER_ITEM_KEY, "Default");
        switch (savedSpinnerSetting) {
            case "Black":
                binding.backgroundSpinner.setSelection(1);
                break;
            case "Red":
                binding.backgroundSpinner.setSelection(2);
                break;
            case "Blue":
                binding.backgroundSpinner.setSelection(3);
                break;
            case "Green":
                binding.backgroundSpinner.setSelection(4);
                break;
            default:
    ...
    public void saveSettings(View view) {
        ...
        if (binding.saveCountSwitch.isChecked()) {
    ...
    public void resetSettings(View view) {
        binding.backgroundSpinner.setSelection(0);
        binding.saveCountSwitch.setChecked(true);
        SharedPreferences.Editor editor = mPreferences.edit();
        editor.putBoolean(COUNT_SAVE_KEY, true);
